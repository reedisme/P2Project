...
def commentOn(user, index):
    if type(index) != int:
        ...
        val = user.posts[index-1].keys
        for i in val:
            user.posts[index - 1][i].append(comment)
        print(user.posts[index-1])
def home(user):
        ...
        if action == str(3):
            ...
            if isuser(friendToView) == True:
                ...
                for i in getObject(friendToView).posts:
                    print(i)
                index = int(input("Select which post you would like to comment on, or press 0 to exit!"))
                if index != 0:
                    commentOn(getObject(friendToView), index)
                else:
                    home(user)
            else:
                print("Bad user")
